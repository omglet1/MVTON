...
if __name__ == "__main__":
    ...
    # 开始测试
    # =============================================================
    if os.path.exists("results/Unpaired_Direst") == False:
        os.makedirs("results/Unpaired_Direst")
    if os.path.exists("results/Unpaired_Concatenation") == False:
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            for i,batch in enumerate(data.dataloader):
                ...
                z_inpaint = model.scale_factor * (encoder_posterior_inpaint.sample()).detach()  #取出了mean这一项
                ...
                x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy() #B,H,W,C
